...
group.by.chore <- function (data, avg.chore.duration) {
  ...
  if (nrow(many) > 0) {
    ...
    many <- merge(many, avg.chore.duration, by = c(), suffixes = c("", ".y"))
    many$mean_log_duration_minutes <- ifelse(
      many$times_completed <= 1,
      many$mean_log_duration_minutes.y,
      many$mean_log_duration_minutes)
    many$sd_log_duration_minutes <- ifelse(
      many$times_completed <= 1,
      many$sd_log_duration_minutes.y,
      many$sd_log_duration_minutes)
    many$mean_log_duration_minutes.y <- NULL
    many$sd_log_duration_minutes.y <- NULL
