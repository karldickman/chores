...
BEGIN
    ...
    SET @minutes_format = '%i:%S';
    SET @hours_format = '%H:%i:%S';
	SELECT chore
			, due_date
            , last_completed
            , TIME_FORMAT(completed_minutes, @minutes_format) AS completed_minutes
            , TIME_FORMAT(remaining_minutes, @minutes_format) AS remaining_minutes
			, TIME_FORMAT(remaining_minutes + 1.282 * stdev_duration_minutes, @minutes_format) AS 90% CI UB
	...
	SELECT category
		...
		, TIME_FORMAT(SUM(backlog_hours), @hours_format) AS backlog_hours
        , TIME_FORMAT(SUM(stdev_backlog_hours), @hours_format) AS stdev_backlog_hours
        , TIME_FORMAT(SUM(backlog_hours) + 1.282 * SUM(stdev_backlog_hours), @hours_format) AS 90% CI UB
